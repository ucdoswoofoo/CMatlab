...
int help(char* file_name) {/*Help File
    ...
    char temp_txt[5] = ".txt";
    strcat_s(temp_route, strlen(file_name), file_name);
    strcat_s(temp_route, strlen(temp_txt), temp_txt);
    FILE* fp;
    char ch;
...
Matrix* Matrix_gen2(int row, int column, Matrix * _mat, MATRIX_TYPE* data) {/*Generate Matrix Struct
    导入_生成矩阵*/
    //Matrix* _mat = (Matrix*)malloc(sizeof(Matrix));
    _mat->row = row;
    _mat->column = column;
...
Matrix* Matrix_gen1(int row, int column, MATRIX_TYPE iniValue) {/*Generate Matrix Struct
    导入_生成矩阵*/
    Matrix* _mat = (Matrix*)malloc(sizeof(Matrix));
    _mat->row = row;
    _mat->column = column;
